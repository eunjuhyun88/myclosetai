...
class EnhancedAIModelAnalyzer:
    ...
    def export_enhanced_json(self, output_file: str = "enhanced_ai_models_analysis.json"):
        ...
        data = {
            ...
            "scan_time": time.time(),
            "search_root": str(self.search_root.absolute()),
            "summary": {
                "total_models": len(self.models),
                "total_size_gb": sum(m.size_mb for m in self.models) / 1024,
                "estimated_memory_gb": sum(m.memory_footprint_estimate for m in self.models) / 1024,
                "duplicate_groups": len(self.duplicate_groups),
                "pytorch_valid_count": sum(1 for m in self.models if m.is_pytorch_valid)
            },
            "models": [],
            "duplicate_groups": dict(self.duplicate_groups),
            "optimization_opportunities": {
                "high_memory_models": [
                    {"name": m.name, "memory_mb": m.memory_footprint_estimate}
                    for m in self.models if m.memory_footprint_estimate > 5000
                ]
            }
        }
        ...
        for model in self.models:
            model_data = {
                ...
                "step_category": model.step_category,
                "is_large": model.is_large,
                "is_ultra_large": model.is_ultra_large,
                "production_priority": model.production_priority,
                "memory_estimate_mb": model.memory_footprint_estimate,
                "optimization_suggestion": model.optimization_suggestion,
                "duplicate_group": model.duplicate_group
            }
            
            if model.metadata:
                model_data["metadata"] = model.metadata
        ...
        print(f"📄 향상된 분석 결과를 {output_file}에 저장했습니다.")

def main():
    """메인 실행 함수"""
    print("🚀 MyCloset AI 향상된 모델 분석기 v2.0")
    print("실제 스캔 데이터(116개 모델, 123.68GB) 기반 최적화")
    print("=" * 80)
    
    search_path = sys.argv[1] if len(sys.argv) > 1 else "."
    
    analyzer = EnhancedAIModelAnalyzer(search_path)
    models = analyzer.scan_and_analyze()
    ...
    analyzer.print_step_production_readiness()
    analyzer.print_optimization_report()
    
    # JSON 내보내기
    analyzer.export_enhanced_json()
    
    print(f"\n✅ 향상된 분석 완료! 총 {len(models)}개 모델 분석")
    print("🎯 프로덕션 배포 전 최적화 권장사항을 확인하세요.")
