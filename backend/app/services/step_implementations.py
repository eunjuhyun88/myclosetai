...
# 초기 로그 메시지
logger.info("=" * 80)
logger.info("🔥 MyCloset AI Step Implementations v10.0 - Logger 초기화 완료")
logger.info("✅ NameError: name 'logger' is not defined 문제 해결")
logger.info("=" * 80)
...
def get_base_step_mixin():
    """BaseStepMixin 동적 import (완전 수정판)"""
    try:
        import importlib
        
        # 실제 파일 구조에 맞는 경로들
        import_paths = [
            'app.ai_pipeline.steps.base_step_mixin',  # 🎯 실제 경로
            'backend.app.ai_pipeline.steps.base_step_mixin',  # 절대 경로
            '.base_step_mixin',  # 현재 패키지 상대 경로
            '..ai_pipeline.steps.base_step_mixin',  # 상위 패키지 상대 경로
        ]
        
        for i, import_path in enumerate(import_paths, 1):
            try:
                logger.debug(f"🔍 BaseStepMixin import 시도 {i}/{len(import_paths)}: {import_path}")
                
                if import_path.startswith('.'):
                    # 상대 import
                    module = importlib.import_module(import_path, package='app.services')
                else:
                    # 절대 import
                ...
                if hasattr(module, 'BaseStepMixin'):
                    ...
                    logger.info(f"✅ BaseStepMixin import 성공: {import_path}")
                    return BaseStepMixin
                else:
                    logger.debug(f"⚠️ {import_path}에 BaseStepMixin 클래스 없음")
                    
            except ImportError as e:
                logger.debug(f"⚠️ {import_path} import 실패: {e}")
                continue
            except Exception as e:
                logger.debug(f"⚠️ {import_path} import 예외: {e}")
                continue
        
        logger.error("❌ 모든 경로에서 BaseStepMixin import 실패")
        return None
        
    except Exception as e:
        logger.error(f"❌ BaseStepMixin import 프로세스 실패: {e}")
        return None

# 기존 함수 이름도 유지 (호환성)
def get_base_step_mixin_class():
    """BaseStepMixin 클래스를 동적으로 가져오기 (호환성 함수)"""
    return get_base_step_mixin()
    ...
    try:
        ...
        for import_path in import_paths:
            try:
                ...
                # 필요한 함수들 가져오기
                get_global_fn = getattr(module, 'get_global_step_factory', None)
                if get_global_fn:
                    factory = get_global_fn()
                    
                    logger.info(f"✅ StepFactory v9.0 동적 import 성공: {import_path}")
                    
                    return {
                        'factory': factory,
                        'StepType': getattr(module, 'StepType', None),
                        'StepCreationResult': getattr(module, 'StepCreationResult', None),
                        'BaseStepMixinConfig': getattr(module, 'BaseStepMixinConfig', None),
                        'BaseStepMixinMapping': getattr(module, 'BaseStepMixinMapping', None),
                        'BaseStepMixinDependencyResolver': getattr(module, 'BaseStepMixinDependencyResolver', None),
                        'BaseStepMixinClassLoader': getattr(module, 'BaseStepMixinClassLoader', None),
                        'StepPriority': getattr(module, 'StepPriority', None),
                        'create_step': getattr(module, 'create_step', None),
                        'create_human_parsing_step': getattr(module, 'create_human_parsing_step', None),
                        'create_pose_estimation_step': getattr(module, 'create_pose_estimation_step', None),
                        'create_cloth_segmentation_step': getattr(module, 'create_cloth_segmentation_step', None),
                        'create_geometric_matching_step': getattr(module, 'create_geometric_matching_step', None),
                        'create_cloth_warping_step': getattr(module, 'create_cloth_warping_step', None),
                        'create_virtual_fitting_step': getattr(module, 'create_virtual_fitting_step', None),
                        'create_post_processing_step': getattr(module, 'create_post_processing_step', None),
                        'create_quality_assessment_step': getattr(module, 'create_quality_assessment_step', None),
                        'create_full_pipeline': getattr(module, 'create_full_pipeline', None),
                        'optimize_conda_environment': getattr(module, 'optimize_conda_environment_for_basestepmixin', None),
                        'validate_step_compatibility': getattr(module, 'validate_basestepmixin_step_compatibility', None),
                        'get_step_info': getattr(module, 'get_basestepmixin_step_info', None)
                    }
        ...
        logger.error("❌ 모든 경로에서 StepFactory import 실패")
        return None
...
# 🔥 그리고 기존 get_body_measurements() 함수 앞에서 
# StepFactory 로딩 부분을 이 새로운 함수로 교체
