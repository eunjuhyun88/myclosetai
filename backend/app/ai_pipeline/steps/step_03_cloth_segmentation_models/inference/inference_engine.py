...
class InferenceEngine:
    ...
    def run_ai_inference(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """🔥 M3 Max 최적화 고도화된 AI 앙상블 의류 분할 추론 시스템"""
        self.logger.info("🚀 M3 Max 최적화 AI 앙상블 의류 분할 시작")
        ...
        try:
            ...
            for model_name, model in self.step.loaded_models.items():
                try:
                    ...
                    if model_name == 'u2net':
                        ...
                        result = self.run_sam_safe_inference(processed_input, model, device_str)
                    ...
                    # 결과 유효성 검증
                    if result and 'segmentation_mask' in result and result['segmentation_mask'] is not None:
                        ensemble_results[model_name] = result['segmentation_mask']
                    ...
                    for model_name, output in ensemble_results.items():
                        if isinstance(output, dict):
                            if 'segmentation_mask' in output:
                                model_outputs_list.append(output['segmentation_mask'])
                            else:
            ...
            return {
                'success': True,
                'segmentation_mask': final_result,
    ...
    def run_u2net_safe_inference(self, input_tensor, model, device_str):
        """U2Net 안전 추론"""
        try:
            with torch.no_grad():
    ...
    def run_sam_safe_inference(self, input_tensor, model, device_str):
        """SAM 안전 추론"""
        try:
            with torch.no_grad():
                if hasattr(input_tensor, 'to'):
    ...
    def run_deeplabv3plus_safe_inference(self, input_tensor, model, device_str):
        """DeepLabV3+ 안전 추론"""
        try:
            with torch.no_grad():
                if hasattr(input_tensor, 'to'):
    ...
    def postprocess_results(self, segmentation_mask):
        """결과 후처리"""
        try:
            if hasattr(self.step, 'postprocessor'):
                return self.step.postprocessor.postprocess(segmentation_mask)
            else:
