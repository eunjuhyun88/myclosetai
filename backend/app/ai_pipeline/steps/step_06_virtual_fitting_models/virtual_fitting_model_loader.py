#!/usr/bin/env python3
"""
🔥 MyCloset AI - Virtual Fitting Model Loader
==============================================
...
class VirtualFittingModelLoader:
    """Virtual Fitting 모델 로더 - 통일된 구조"""
    
    def __init__(self):
        self.supported_models = [
            'realvisxl', 'vitonhd', 'ootd', 'virtual_tryon', 'fitting_network'
        ]
        self.model_paths = {}
            ...
            if ai_models_root.exists():
                for checkpoint_file in ai_models_root.rglob("*.pth"):
                    if any(keyword in checkpoint_file.name.lower() for keyword in 
                          ['realvisxl', 'vitonhd', 'ootd', 'virtual', 'fitting', 'tryon']):
                        self.model_paths[checkpoint_file.stem] = str(checkpoint_file)
                        logger.info(f"✅ Virtual Fitting 모델 발견: {checkpoint_file.name}")
