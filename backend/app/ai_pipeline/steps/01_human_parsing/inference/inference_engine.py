...
class InferenceEngine:
    ...
    def run_graphonomy_safe_inference(self, input_tensor: torch.Tensor, model: nn.Module, device: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return self.create_error_response(f"Graphonomy 추론 실패: {e}")
    ...
    def run_deeplabv3plus_safe_inference(self, input_tensor: torch.Tensor, model: nn.Module, device: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"⚠️ DeepLabV3+ 추론 실패: {e}")
            return self.create_error_response(f"DeepLabV3+ 추론 실패: {e}")
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if isinstance(parsing_output, torch.Tensor):
                    ...
                    # U2Net은 보통 sigmoid 출력을 사용
                    if parsing_output.shape[0] == 1:
                        parsing_output = torch.sigmoid(parsing_output)
            ...
            return self.create_error_response(f"U2Net 추론 실패: {e}")
