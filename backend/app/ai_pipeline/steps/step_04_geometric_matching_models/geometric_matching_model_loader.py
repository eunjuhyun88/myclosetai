#!/usr/bin/env python3
"""
🔥 MyCloset AI - Geometric Matching Model Loader
================================================
...
class GeometricMatchingModelLoader:
    """Geometric Matching 모델 로더 - 통일된 구조"""
    
    def __init__(self):
        self.supported_models = [
            'gmm', 'geometric_matching', 'feature_matching', 'correspondence'
        ]
        self.model_paths = {}
        ...
        try:
            ...
            if ai_models_root.exists():
                for checkpoint_file in ai_models_root.rglob("*.pth"):
                    if "geometric" in checkpoint_file.name.lower() or "gmm" in checkpoint_file.name.lower():
                        self.model_paths[checkpoint_file.stem] = str(checkpoint_file)
                        logger.info(f"✅ Geometric Matching 모델 발견: {checkpoint_file.name}")
