...
🔥 MyCloset AI Step 01 - Human Parsing (ModelLoader 완전 연동 + 폴백 제거)
✅ 시뮬레이션 모드 완전 제거 - 100% 실제 AI 모델만 사용
✅ ModelLoader 실패 시 명확한 에러 반환 (폴백 없음)
...
- ModelLoader.get_model()을 통한 실제 AI 모델만 사용
- 모든 추론이 ModelLoader가 제공한 모델로만 실행
- 실제 모델 로드 실패 시 즉시 에러 반환
"""
...
try:
    ...
    raise ImportError(f"❌ ModelLoader는 필수 의존성입니다: {e}")

# 🔥 BaseStepMixin 연동 (완전 수정) - 순환 참조 방지 + MRO 해결
try:
    ...
    logger.error(f"❌ BaseStepMixin 임포트 실패: {e}")
    
    # 🔥 MRO 안전한 폴백 클래스 - 다중 상속 오류 방지
    class BaseStepMixin:
        """MRO 안전한 폴백 BaseStepMixin - 순환 참조 및 다중 상속 오류 해결"""
        
        def __init__(self, *args, **kwargs):
            # 🔥 super().__init__() 호출하지 않음 - MRO 오류 방지
            ...
            self.logger.info(f"🔧 폴백 BaseStepMixin 초기화 완료: {self.step_name}")
...
try:
    ...
    logger.warning("⚠️ 유틸리티 모듈 일부 사용 불가")
...
class HumanParsingConfig:
    ...
    strict_mode: bool = True  # 🔥 실패 시 즉시 에러 반환
...
class HumanParsingStep(BaseStepMixin):
    ...
    ✅ strict_mode=True로 실패 시 즉시 에러 반환
    """
        ...
        **kwargs
        ...
        if not MODEL_LOADER_AVAILABLE:
            if self.config.strict_mode:
                raise RuntimeError("❌ strict_mode에서 ModelLoader 필수이지만 사용할 수 없습니다")
            else:
                logger.warning("⚠️ ModelLoader 사용 불가 - strict_mode가 False이므로 계속 진행")
        ...
        self.logger.info(f"🚨 Strict Mode: {'✅' if self.config.strict_mode else '❌'}")
    ...
    def _setup_config_safe(
        ...
        try:
            ...
            return HumanParsingConfig(
                ...
                strict_mode=kwargs.get('strict_mode', True)  # 🔥 기본값 True
            )
    ...
    def _setup_model_interface_safe(self, model_loader=None):
        ...
        try:
            if not MODEL_LOADER_AVAILABLE:
                if self.config.strict_mode:
                    raise RuntimeError("❌ strict_mode: ModelLoader가 필수이지만 사용할 수 없습니다")
                self.logger.warning("⚠️ ModelLoader 사용 불가능")
                self.model_interface = None
                return
            ...
            if model_loader is None:
                ...
                try:
                    ...
                    if self.config.strict_mode:
                        raise RuntimeError(f"❌ strict_mode: 전역 ModelLoader 획득 실패: {e}")
            ...
            if model_loader and hasattr(model_loader, 'create_step_interface'):
                try:
                    ...
                    if self.config.strict_mode:
                        raise RuntimeError(f"❌ strict_mode: 모델 인터페이스 생성 실패: {e}")
                    self.logger.warning(f"⚠️ 모델 인터페이스 생성 실패: {e}")
                    self.model_interface = None
            else:
                if self.config.strict_mode:
                    raise RuntimeError("❌ strict_mode: ModelLoader를 찾을 수 없습니다")
                self.logger.warning("⚠️ ModelLoader를 찾을 수 없음")
                self.model_interface = None
            
        except Exception as e:
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: ModelLoader 인터페이스 설정 실패: {e}")
    ...
    def _validate_model_interface(self):
        ...
        try:
            if self.model_interface:
                ...
                for method in required_methods:
                    if not hasattr(self.model_interface, method):
                        error_msg = f"ModelLoader 인터페이스에 {method} 메서드 없음"
                        if self.config.strict_mode:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: ModelLoader 인터페이스 검증 실패: {e}")
    ...
    async def initialize(self) -> bool:
        """
        ✅ Step 초기화 - ModelLoader를 통한 실제 AI 모델 로드 (strict_mode 적용)
        
        Returns:
            bool: 초기화 성공 여부
        
        Raises:
            RuntimeError: strict_mode에서 모델 로드 실패 시
        """
        try:
            self.logger.info("🔄 1단계: 인체 파싱 모델 초기화 중...")
            
            if not MODEL_LOADER_AVAILABLE or not self.model_interface:
                error_msg = "ModelLoader 사용 불가능"
                if self.config.strict_mode:
                    raise RuntimeError(f"❌ strict_mode: {error_msg}")
                self.logger.warning(f"⚠️ {error_msg}")
                self.is_initialized = True
            ...
            success = await self._load_models_from_model_loader()
            
            if self.config.strict_mode and not success:
                raise RuntimeError("❌ strict_mode: 모델 로드 실패")
            
            # === 모델 워밍업 ===
            if self.config.warmup_enabled and success:
                await self._warmup_models_safe()
            
            # === M3 Max 최적화 적용 ===
            if self.device == 'mps' and success:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 1단계 초기화 실패: {e}")
            # strict_mode가 False인 경우에만 부분 실패 허용
            self.is_initialized = True
            return False
    
    async def _load_models_from_model_loader(self) -> bool:
        """🔥 핵심! ModelLoader를 통한 실제 AI 모델 로드"""
        success = False
        try:
            # === 주 모델 로드 (Graphonomy) ===
            primary_success = await self._load_primary_model_from_loader()
            
            # === 백업 모델 로드 (U²-Net) ===
            backup_success = await self._load_backup_model_from_loader()
            ...
            if self.config.strict_mode and not success:
                raise RuntimeError("❌ strict_mode: 모든 모델 로드 실패")
            
            return success
            
        except Exception as e:
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: ModelLoader를 통한 모델 로드 실패: {e}")
            self.logger.warning(f"⚠️ ModelLoader를 통한 모델 로드 실패: {e}")
            return False
    
    async def _load_primary_model_from_loader(self) -> bool:
        ...
        try:
            ...
            if model is not None:
                ...
                error_msg = f"ModelLoader에서 주 모델 반환 실패: {self.config.model_name}"
                if self.config.strict_mode:
                    raise RuntimeError(f"❌ strict_mode: {error_msg}")
                self.logger.warning(f"⚠️ {error_msg}")
                return False
            ...
            error_msg = f"주 모델 로드 오류: {e}"
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: {error_msg}")
            self.logger.warning(f"⚠️ {error_msg}")
            return False
    
    async def _load_backup_model_from_loader(self) -> bool:
    ...
    async def process(
        ...
        **kwargs
        ...
        """
        ✅ 메인 처리 함수 - ModelLoader를 통한 실제 AI 인체 파싱 + 시각화 (strict_mode 적용)
        ...
        Raises:
            RuntimeError: strict_mode에서 처리 실패 시
        """
        
        if not self.is_initialized:
            error_msg = "모델이 초기화되지 않음"
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: {error_msg}")
            self.logger.warning(f"⚠️ {error_msg} - 자동 초기화 시도")
        ...
        try:
            ...
            error_msg = f"1단계 처리 실패: {e}"
            self.logger.error(f"❌ {error_msg}")
            
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: {error_msg}")
            
            # strict_mode가 False인 경우에만 폴백 결과 반환
        ...
        try:
            ...
            if 'backup' in self.models_loaded:
                ...
                try:
                    ...
                    error_msg = f"백업 모델 추론도 실패: {e}"
                    if self.config.strict_mode:
                        raise RuntimeError(f"❌ strict_mode: {error_msg}")
                    self.logger.warning(f"⚠️ {error_msg}")
            
            # === 모든 실제 모델이 실패한 경우 ===
            error_msg = "모든 실제 AI 모델 실패"
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: {error_msg}")
        ...
        try:
            def _postprocess_sync():
                try:
                    ...
                    if self.config.strict_mode:
                        raise RuntimeError(f"❌ strict_mode: 동기 후처리 실패: {e}")
            ...
            try:
                ...
                if self.config.strict_mode:
                    raise RuntimeError(f"❌ strict_mode: 비동기 후처리 실패: {e}")
        ...
        try:
            ...
            def _create_visualizations_safe():
                try:
                    ...
                    if self.config.strict_mode:
                        raise RuntimeError(f"❌ strict_mode: 시각화 생성 실패: {e}")
            ...
            try:
                ...
                if self.config.strict_mode:
                    raise RuntimeError(f"❌ strict_mode: 비동기 시각화 실패: {e}")
                self.logger.warning(f"⚠️ 비동기 시각화 실패: {e}")
                return _create_visualizations_safe()
                
        except Exception as e:
            error_msg = f"시각화 생성 완전 실패: {e}"
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: {error_msg}")
            self.logger.error(f"❌ {error_msg}")
            return {"colored_parsing": "", "overlay_image": "", "legend_image": ""}
    ...
    def _tensor_to_pil_safe(self, tensor: torch.Tensor) -> Image.Image:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 텐서->PIL 변환 실패: {e}")
    ...
    def _create_colored_parsing_map_safe(self, parsing_map: np.ndarray) -> Image.Image:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 컬러 파싱 맵 생성 실패: {e}")
    ...
    def _create_overlay_image_safe(self, original_pil: Image.Image, colored_parsing: Image.Image) -> Image.Image:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 오버레이 생성 실패: {e}")
    ...
    def _create_legend_image_safe(self, parsing_map: np.ndarray) -> Image.Image:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 범례 생성 실패: {e}")
            self.logger.warning(f"⚠️ 범례 생성 실패: {e}")
            # 기본 범례 이미지
    ...
    def _pil_to_base64_safe(self, pil_image: Image.Image) -> str:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: base64 변환 실패: {e}")
    ...
    def _apply_morphological_operations_safe(self, parsing_map: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 모폴로지 연산 실패: {e}")
    ...
    def _create_body_masks_safe(self, parsing_map: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 전체 마스크 생성 실패: {e}")
    ...
    def _analyze_clothing_regions_safe(self, parsing_map: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 의류 영역 분석 실패: {e}")
    ...
    def _calculate_confidence_safe(self, model_output: torch.Tensor) -> float:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 신뢰도 계산 실패: {e}")
    ...
    def _get_detected_parts_safe(self, parsing_map: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 전체 부위 정보 수집 실패: {e}")
    ...
    def _get_bounding_box_safe(self, mask: np.ndarray) -> Dict[str, int]:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 바운딩 박스 계산 실패: {e}")
    ...
    def _get_centroid_safe(self, mask: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 중심점 계산 실패: {e}")
    ...
    def _create_fallback_result_safe(
        ...
        """안전한 폴백 결과 생성 (에러 발생 시) - strict_mode에서는 사용되지 않음"""
        try:
            return {
                "success": False,
    ...
    async def get_model(self, model_name: Optional[str] = None) -> Optional[Any]:
        ...
        try:
            if not self.model_interface:
                error_msg = "ModelLoader 인터페이스가 없습니다"
                if self.config.strict_mode:
                    raise RuntimeError(f"❌ strict_mode: {error_msg}")
                self.logger.warning(f"⚠️ {error_msg}")
                return None
            ...
            if self.config.strict_mode:
                raise RuntimeError(f"❌ strict_mode: 모델 로드 실패: {e}")
    ...
    def get_processing_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # ModelLoader 성공률 계산
            total_model_attempts = stats.get('real_model_calls', 0)
            if total_model_attempts > 0:
                stats['model_loader_success_rate'] = 100.0  # 실제 모델만 사용하므로
            else:
...
async def create_human_parsing_step(
    device: str = "auto",
    config: Optional[Union[Dict[str, Any], HumanParsingConfig]] = None,
    strict_mode: bool = True,  # 🔥 기본값 True
    **kwargs
) -> HumanParsingStep:
    """
    ...
    Raises:
        RuntimeError: strict_mode에서 초기화 실패 시
    """
    ...
    try:
        ...
        default_config = HumanParsingConfig(
            ...
            optimization_enabled=kwargs.get('optimization_enabled', True),
            quality_level=kwargs.get('quality_level', 'balanced'),
            strict_mode=strict_mode  # 🔥 strict_mode 설정
        )
        ...
        if strict_mode and not success:
            raise RuntimeError("❌ strict_mode: Step 01 초기화 실패")
        ...
        step = HumanParsingStep(device='cpu', config=HumanParsingConfig(strict_mode=False))
...
def create_human_parsing_step_sync(
    ...
    **kwargs
    ...
    try:
        ...
        return HumanParsingStep(device='cpu', config=HumanParsingConfig(strict_mode=False))
...
async def test_all_features():
    ...
    try:
        ...
        print("🔄 13. 모델 정보 테스트")
        loaded_models = step.get_loaded_models()
        print(f"   🤖 로드된 모델: {loaded_models}")
        ...
        # 정리
        await step.cleanup()
        print("✅ 모든 기능 테스트 완료! (17개 기능 모두 확인)")
...
if __name__ == "__main__":
    # 모든 기능 테스트 실행
    print("="*80)
    print("🎯 완전한 step_01_human_parsing.py 테스트 (모든 누락 기능 포함)")
    print("="*80)
...
# 모듈 로딩 확인
logger.info("✅ Step 01 Human Parsing 모듈 로드 완료 - ModelLoader 완전 연동 + 폴백 제거 버전")
logger.info("🔗 시뮬레이션 모드 완전 제거 - 100% 실제 AI 모델만 사용")
logger.info("🚀 ModelLoader.get_model()을 통한 실제 AI 모델 로드")
logger.info("🎯 모든 추론이 ModelLoader가 제공한 모델로만 실행")
logger.info("🚨 strict_mode=True로 실패 시 즉시 에러 반환")
