...
# 🔥 새로운 모듈화된 ClothSegmentationStep import
try:
    from .models.step import ClothSegmentationStep as ModularClothSegmentationStep
    print("✅ 모듈화된 ClothSegmentationStep import 성공")
    MODULAR_STEP_AVAILABLE = True
except ImportError as e:
    print(f"⚠️ 모듈화된 ClothSegmentationStep import 실패: {e}")
    ModularClothSegmentationStep = None
    MODULAR_STEP_AVAILABLE = False
...
class ClothSegmentationStep(BaseStepMixin):
    ...
    def _load_segmentation_models_via_central_hub(self):
        """Central Hub를 통한 세그멘테이션 모델 로딩 - 통합 로더 사용"""
        try:
            ...
            # 통합 로더 인스턴스 생성
            integrated_loader = create_cloth_segmentation_integrated_loader()
            ...
            # 통합 로더로 모델 로딩
            load_result = integrated_loader.load_models_integrated(target_methods)
            
            if load_result['success']:
                # 로드된 모델들을 기존 구조에 맞게 설정
                self.loaded_models = load_result['loaded_models']
                self.available_methods = load_result['available_methods']
                
                # 모델 로딩 상태 업데이트
                for method_name in self.available_methods:
                    if method_name == 'u2net_cloth':
                        self.models_loading_status['u2net_cloth'] = True
                        self.ai_models['u2net'] = self.loaded_models[method_name]
                        self.segmentation_models['u2net'] = self.loaded_models[method_name]
                    elif method_name == 'sam_huge':
                        self.models_loading_status['sam_huge'] = True
                        self.ai_models['sam'] = self.loaded_models[method_name]
                        self.segmentation_models['sam'] = self.loaded_models[method_name]
                    elif method_name == 'deeplabv3_plus':
                        self.models_loading_status['deeplabv3plus'] = True
                        self.ai_models['deeplabv3plus'] = self.loaded_models[method_name]
                        self.segmentation_models['deeplabv3plus'] = self.loaded_models[method_name]
                
                self.models_loading_status['total_loaded'] = len(self.loaded_models)
                
                self.logger.info(f"✅ 통합 로더로 {len(self.loaded_models)}개 모델 로딩 완료")
                return True
            else:
                self.logger.warning(f"⚠️ 통합 로더 로딩 실패: {load_result['errors']}")
                self.models_loading_status['loading_errors'] = load_result['errors']
                return False
                
        except Exception as e:
            self.logger.error(f"❌ 통합 로더 로딩 실패: {str(e)}")
            self.models_loading_status['loading_errors'].append(str(e))
            return False
...
def create_cloth_segmentation_step_integrated(**kwargs) -> 'ClothSegmentationStepIntegrated':
    ...
    try:
        ...
        logger.warning(f"통합 스텝 import 실패, 기존 스텝 사용: {e}")
        return ClothSegmentationStep(**kwargs)
