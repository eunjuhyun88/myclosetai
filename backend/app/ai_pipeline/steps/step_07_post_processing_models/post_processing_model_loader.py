...
class PostProcessingModelLoader:
    ...
    def load_model(self, model_type: str, **kwargs) -> nn.Module:
        ...
        try:
            ...
            # 체크포인트 로드 (있는 경우)
            model = self._load_checkpoint(model_type, model)
    ...
    def _load_checkpoint(self, model_type: str, model: nn.Module) -> nn.Module:
        """
        체크포인트를 로드합니다.
        ...
        Returns:
            체크포인트가 로드된 모델
        """
        checkpoint_path = self.checkpoint_dir / f"{model_type}_checkpoint.pth"
        ...
        if checkpoint_path.exists():
            ...
            logger.info(f"No checkpoint found for {model_type}, using random weights")
