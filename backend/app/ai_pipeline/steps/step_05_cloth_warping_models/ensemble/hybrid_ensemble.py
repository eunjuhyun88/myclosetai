#!/usr/bin/env python3
"""
🔥 MyCloset AI - Cloth Warping Hybrid Ensemble System
=======================================================

🎯 다중 모델 앙상블을 통한 정확도 향상
✅ 8개 Cloth Warping 모델 통합
✅ M3 Max 최적화
✅ 메모리 효율적 처리
✅ 품질 기반 가중치 조정
"""
...
class ClothWarpingEnsembleSystem(nn.Module):
    """
    🔥 Cloth Warping 앙상블 시스템
    
    다중 모델의 출력을 통합하여 최종 의류 왜곡 결과를 생성합니다.
    """
    ...
    def __init__(self, config: EnsembleConfig = None):
        ...
        self.logger.info(f"🎯 Cloth Warping 앙상블 시스템 초기화 (디바이스: {self.device})")
        ...
        self.quality_metrics = {
            ...
            "warping_accuracy": 0.0,
            "deformation_quality": 0.0
        }
        
        self.logger.info("✅ Cloth Warping 앙상블 시스템 초기화 완료")
    ...
    def _update_quality_metrics(self, ensemble_output: torch.Tensor, 
        ...
        # 왜곡 정확도 계산
        warping_accuracy = self._calculate_warping_accuracy(ensemble_output)
        self.quality_metrics["warping_accuracy"] = warping_accuracy
        
        # 변형 품질 계산
        deformation_quality = self._calculate_deformation_quality(ensemble_output)
        self.quality_metrics["deformation_quality"] = deformation_quality
    ...
    def _calculate_warping_accuracy(self, output: torch.Tensor) -> float:
        """왜곡 정확도 계산"""
        if output.dim() == 0:
            return 0.0
    ...
    def _calculate_deformation_quality(self, output: torch.Tensor) -> float:
        """변형 품질 계산"""
        if output.dim() == 0:
            return 0.0
        ...
        if output.dim() > 2:
            ...
            for b in range(batch_size):
                ...
                if output.size(1) > 1:  # 채널이 있는 경우
                    deformation_field = output[b].mean(dim=0)  # 채널 평균
                else:
                    deformation_field = output[b]
                
                # 변형 품질 점수 계산
                if deformation_field.dim() == 2:
                    # 2D 텐서를 numpy로 변환
                    field_np = deformation_field.detach().cpu().numpy()
                    
                    # 변형의 부드러움 계산 (gradient 기반)
                    # x 방향 gradient
                    grad_x = np.gradient(field_np, axis=1)
                    # y 방향 gradient
                    grad_y = np.gradient(field_np, axis=0)
                    
                    # gradient의 크기
                    gradient_magnitude = np.sqrt(grad_x**2 + grad_y**2)
                    
                    # 부드러운 변형일수록 낮은 gradient (높은 품질)
...
# 앙상블 시스템 인스턴스 생성 함수
def create_cloth_warping_ensemble(config: EnsembleConfig = None) -> ClothWarpingEnsembleSystem:
    """Cloth Warping 앙상블 시스템 생성"""
    return ClothWarpingEnsembleSystem(config)

# 기본 설정으로 앙상블 시스템 생성
def create_default_ensemble() -> ClothWarpingEnsembleSystem:
    """기본 설정으로 앙상블 시스템 생성"""
...
if __name__ == "__main__":
    ...
    # 테스트 데이터 생성
    batch_size, channels, height, width = 2, 2, 256, 256  # 2채널 (x, y 변형)
    test_outputs = [
