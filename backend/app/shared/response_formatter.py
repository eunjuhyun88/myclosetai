# backend/app/shared/response_formatter.py
"""
🔥 MyCloset AI Response Formatter
================================================================================

API 응답 포맷팅을 위한 공통 모듈입니다.

- format_step_api_response: 스텝별 API 응답 포맷팅
- format_api_response: 일반 API 응답 포맷팅
...
def format_step_api_response(
    ...
    **kwargs
    ...
    """스텝별 API 응답 형식화 (프론트엔드 호환) - Central Hub 기반"""
...
def enhance_step_result_for_frontend(result: Dict[str, Any], step_id: int) -> Dict[str, Any]:
    ...
    # 진행률 정보 추가
    enhanced["progress_percentage"] = (step_id / 8) * 100
    enhanced["current_step"] = step_id
    enhanced["total_steps"] = 8
    enhanced["remaining_steps"] = max(0, 8 - step_id)
