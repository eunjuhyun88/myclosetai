...
"""
MyCloset AI Backend - M3 Max 128GB 최적화 Pydantic V2 호환 스키마
프론트엔드 mycloset-uiux.tsx와 완전 호환
pipeline_manager.py와 완전 통합
"""
...
class VirtualTryOnRequest(BaseConfigModel):
    ...
    def validate_image_input(self) -> Self:
        ...
        if not any(person_sources):
            raise ValueError('사용자 이미지(person_image_data 또는 person_image_url)가 필요합니다')
        
        if not any(clothing_sources):
            raise ValueError('의류 이미지(clothing_image_data 또는 clothing_image_url)가 필요합니다')
    ...
    def optimize_for_m3_max(self) -> Self:
        ...
        # M3 Max 전용 모드 설정
        if self.quality_level == QualityLevelEnum.ULTRA:
...
class ClothingAnalysis(BaseConfigModel):
    ...
    dominant_color: List[int] = Field(..., description="주요 색상 [R, G, B]", min_length=3, max_length=3)
...
class VirtualTryOnResponse(BaseConfigModel):
    ...
    # 기존 호환성 필드들
    processing_time: Optional[PositiveFloat] = Field(None, description="처리 시간 (초)")
...
PipelineProgress = ProcessingStatus
