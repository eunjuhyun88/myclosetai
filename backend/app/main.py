# =============================================================================
# backend/app/main.py - 기존 완전 기능 유지 + App.tsx 호환성 개선
# =============================================================================

"""
🔥 MyCloset AI FastAPI 서버 - 기존 완전 기능 유지 + App.tsx 호환
✅ 기존 로그 시스템 완전 유지 (중복 해결 포함)
✅ 기존 AI 파이프라인 완전 유지 (4단계 폴백)
✅ 기존 WebSocket 시스템 완전 유지
✅ 기존 모든 API 엔드포인트 완전 유지
✅ App.tsx API 호출과 완전 호환되도록 개선
✅ 세션 기반 이미지 재업로드 문제 해결
✅ M3 Max 최적화 완전 유지
✅ conda 환경 완벽 지원
"""
...
print(f"🍎 M3 Max 설정 강제 적용됨")  # App.tsx 호환 메시지 추가
print(f"🔍 백엔드 루트: {backend_root}")
print(f"📁 작업 디렉토리: {os.getcwd()}")

# =============================================================================
# 🔥 Step 2: 필수 라이브러리 import (기존 코드 완전 유지)
# =============================================================================
...
try:
    ...
    print(f"❌ PIL 라이브러리 import 실패: {e}")

# =============================================================================
# 🔥 Step 2.5: AI 파이프라인 import (기존 코드 완전 유지)
# =============================================================================
...
# =============================================================================
# 🔥 Step 2.6: 완전한 로깅 시스템 설정 (기존 코드 완전 유지)
# =============================================================================
...
log_system_event("STARTUP", "MyCloset AI 백엔드 시작 - App.tsx 완전 호환 + 기존 기능 완전 유지")

# =============================================================================
# 🔥 Step 3: 데이터 모델 정의 (기존 + App.tsx 호환)
# =============================================================================
...
# =============================================================================
# 🔥 Step 4: 글로벌 변수 및 상태 관리 (기존 코드 완전 유지)
# =============================================================================
...
# =============================================================================
# 🔥 Step 5: AI 파이프라인 초기화 (기존 코드 완전 유지)
# =============================================================================

async def initialize_ai_pipeline() -> bool:
    """기존 완전한 AI 파이프라인 초기화 함수 (4단계 폴백 유지)"""
...
# =============================================================================
# 🔥 기존 실제 AI 처리 함수들 + App.tsx 호환성 개선
# =============================================================================

def create_session() -> str:
    """새 세션 ID 생성 (App.tsx 호환)"""
...
async def send_websocket_update(session_id: str, step: int, progress: int, message: str):
    """WebSocket으로 진행률 업데이트 전송 (App.tsx 완전 호환)"""
...
async def process_upload_validation(person_image: UploadFile, clothing_image: UploadFile) -> StepResult:
    """Step 1: 이미지 업로드 검증 + 실제 AI 처리 (App.tsx 완전 호환)"""
    ...
    try:
        ...
        # 🔥 핵심: Base64 인코딩 및 세션 저장 (App.tsx 재업로드 문제 해결)
                ...
                if person_tensor is not None:
                    ...
                    if parsing_result.get('success', False):
                        ...
                        result = StepResult(
                            ...
                            details={
                                "session_id": session_id,  # App.tsx 필수
        ...
        result = StepResult(
            ...
            confidence=0.95,
            details={
                "session_id": session_id,  # 🔥 App.tsx에서 필수로 사용
                "person_image_size": len(person_data),
                "clothing_image_size": len(clothing_data),
                "image_format": "JPEG",
                "result_image": person_b64[:100] + "...",  # 미리보기
                "overlay_image": clothing_b64[:100] + "...",  # 미리보기
                "detected_parts": 18,
                "total_parts": 20,
                "body_parts": ["머리", "상체", "하체", "팔", "다리"],
                "ai_processing": False,
...
async def process_measurements_validation(height: float, weight: float, session_id: str) -> StepResult:
    """Step 2: 신체 측정값 검증 (App.tsx 완전 호환)"""
    ...
    try:
        ...
        # 🔥 핵심: 측정값을 세션에 저장 (App.tsx Step 3-8에서 사용)
        ...
        result = StepResult(
            ...
            confidence=0.98,
            details={
                "session_id": session_id,  # App.tsx 호환
                "height": height,
                "weight": weight,
                "bmi": round(bmi, 1),
                "valid_range": True,
                "body_type": "표준" if 18.5 <= bmi <= 25 else "비표준"
            }
        )
...
async def process_step_with_ai(step_num: int, session_id: str, step_data: Dict[str, Any] = None) -> StepResult:
    """범용 AI 처리 함수 (Step 3-8) - App.tsx 완전 호환"""
    step_names = {
        3: "인체 파싱",
        4: "포즈 추정", 
        5: "의류 분석",
    ...
    try:
        ...
        # 🔥 핵심: 이미지는 세션에서 가져오기 (App.tsx 재업로드 방지)
        if "images" not in session:
            raise HTTPException(status_code=400, detail="이미지 데이터가 세션에 없습니다")
        
        # WebSocket 진행률 업데이트 (App.tsx 호환)
        await send_websocket_update(session_id, step_num, 50, f"{step_name} 처리 중...")
        ...
        if ai_result and ai_result.get('success', False):
            ...
            # Step 7 특별 처리 (가상 피팅 결과) - App.tsx 호환
            ...
            # 시뮬레이션 처리 (App.tsx 호환)
            processing_times = {3: 1.2, 4: 0.8, 5: 0.6, 6: 1.5, 7: 2.5, 8: 0.3}
            await asyncio.sleep(processing_times.get(step_num, 1.0))
            processing_time = (datetime.now() - start_time).total_seconds()
            ...
            result = StepResult(
                ...
                processing_time=processing_time,
                confidence=0.85 + step_num * 0.02,
                details={
                    "session_id": session_id,  # App.tsx 호환
                    "result_image": session["images"]["person_image"][:100] + "...",
                    "overlay_image": session["images"]["clothing_image"][:100] + "...",
                    "ai_processing": False,
                    "simulation_mode": True,
                    **_get_step_specific_details(step_num)
                }
            )
            
            # Step 7 시뮬레이션 특별 처리 (App.tsx 호환)
            if step_num == 7:
                result.fitted_image = session["images"]["person_image"]  # 원본 이미지 사용
        ...
        # WebSocket 완료 알림 (App.tsx 호환)
        await send_websocket_update(session_id, step_num, 100, f"{step_name} 완료")
...
def _get_step_specific_details(step_num: int) -> Dict[str, Any]:
    """단계별 특화 정보 반환 (App.tsx 시각화용)"""
    details_map = {
        3: {
            "detected_parts": 18,
            "total_parts": 20,
    ...
    return details_map.get(step_num, {})

# =============================================================================
# 🔥 FastAPI 애플리케이션 생명주기 관리 (기존 코드 완전 유지)
# =============================================================================

@asynccontextmanager
async def lifespan(app: FastAPI):
    """애플리케이션 생명주기 관리 (기존 코드 완전 유지)"""
    
    # ===== 시작 단계 =====
    try:
        log_system_event("STARTUP_BEGIN", "FastAPI 앱 시작")
        
        # AI 파이프라인 초기화 (기존 함수 사용)
...
# =============================================================================
# 🔥 FastAPI 애플리케이션 생성 (기존 설정 + App.tsx 호환)
# =============================================================================

app = FastAPI(
    title="MyCloset AI",
    description="AI 기반 가상 피팅 서비스 - 기존 완전 기능 + App.tsx 호환",
    version="3.0.0",
    lifespan=lifespan
)
...
async def root():
    """루트 엔드포인트"""
    return {
        "message": "MyCloset AI Server - App.tsx 완전 호환",
        "status": "running",
        "version": "3.0.0",
        "docs": "/docs",
...
async def step_1_upload_validation(
    ...
    """Step 1: 이미지 업로드 검증 (App.tsx 정확 매칭)"""
    try:
...
async def step_2_measurements_validation(
    ...
    """Step 2: 신체 측정값 검증 (App.tsx 정확 매칭)"""
    try:
...
async def step_3_human_parsing(session_id: str = Form(...)) -> StepResult:
    """Step 3: 인체 파싱 (App.tsx 정확 매칭)"""
    try:
        logger.info(f"🔍 Step 3: 인체 파싱 시작 (세션: {session_id})")
        
        await send_websocket_update(session_id, 3, 30, "AI 인체 파싱 중...")
        result = await process_step_with_ai(3, session_id)
        await send_websocket_update(session_id, 3, 100, "인체 파싱 완료")
...
# =============================================================================
# 🔥 통합 파이프라인 및 WebSocket (기존 기능 + App.tsx 호환)
# =============================================================================
...
async def complete_pipeline(
    ...
    try:
        ...
        fit_score = step7_result.get("fit_score", 0.88) if step7_result else 0.88
        
        final_result = TryOnResult(
            success=True,
            message="전체 파이프라인 완료" + (" (실제 AI 처리)" if AI_MODULES_AVAILABLE else " (시뮬레이션)"),
            processing_time=7.5,
            confidence=0.90,
...
# =============================================================================
# 🔥 WebSocket 엔드포인트 (App.tsx 완전 호환)
# =============================================================================

@app.websocket("/api/ws/pipeline")
async def websocket_pipeline(websocket: WebSocket):
    """파이프라인 진행률 WebSocket (App.tsx 완전 호환)"""
...
async def get_pipeline_steps():
    ...
    steps = [
        ...
        {"step": 3, "name": "인체 파싱", "description": "AI 기반 인체 영역 분할"},
...
# =============================================================================
# 🔥 서버 실행 정보 출력 (기존 + App.tsx 호환 메시지 추가)
# =============================================================================
