# create_smart_mapper.py - backend 디렉토리에서 실행
"""
SmartModelPathMapper 파일 생성 스크립트
실행: python create_smart_mapper.py
"""

import os
from pathlib import Path

# SmartModelPathMapper 코드
smart_mapper_content = '''# backend/app/ai_pipeline/utils/smart_model_mapper.py
"""
...
class SmartModelPathMapper:
    ...
    def _create_unified_mappings(self) -> Dict[str, Dict[str, Any]]:
        """🔥 통합 모델 매핑 테이블 생성 - 워닝 모델들 포함"""
        return {
            # Step 05: Cloth Warping - 워닝 모델들
            "realvis_xl": {
                ...
                "search_patterns": [
                    ...
                    "cloth_warping_model/RealVisXL_V4.0.safetensors"
                ],
            ...
            "vgg16_warping": {
                ...
                "fallback_patterns": ["**/vgg16*warp*.pth", "**/vgg16*.pth"],
                "min_size_mb": 500,
                "priority": 2
            },
            ...
            "vgg19_warping": {
                ...
                "fallback_patterns": ["**/vgg19*warp*.pth", "**/vgg19*.pth"],
                "min_size_mb": 500,
                "priority": 3
            },
            ...
            # Step 04: Geometric Matching - GMM 워닝
            "gmm": {
                ...
                "search_patterns": [
                    ...
                    "geometric_matching_model/gmm_final.pth"
                ],
            ...
            "ootdiffusion": {
                ...
                "search_patterns": [
                    ...
                    "virtual_fitting_diffusion/diffusion_pytorch_model.*"
                ],
            ...
            "quality_assessment_clip": {
                ...
                "search_patterns": [
                    ...
                    "ViT-L-14.pt",
                    "quality_assessment_clip/open_clip_pytorch_model.bin"
                ],
...
def get_step_model_paths(step_class: str) -> Dict[str, Path]:
    ...
    for model_name, config in mapper.unified_model_mappings.items():
...
def create_smart_mapper_file():
    """SmartModelPathMapper 파일 생성"""
    try:
        # 디렉토리 생성
        target_dir = Path("app/ai_pipeline/utils")
        target_dir.mkdir(parents=True, exist_ok=True)
        
        # 파일 생성
        target_file = target_dir / "smart_model_mapper.py"
        
        with open(target_file, 'w', encoding='utf-8') as f:
            f.write(smart_mapper_content)
        
        print(f"✅ SmartModelPathMapper 생성 완료: {target_file}")
        return True
        
    except Exception as e:
        print(f"❌ SmartModelPathMapper 생성 실패: {e}")
        return False

if __name__ == "__main__":
    print("🔧 SmartModelPathMapper 파일 생성 중...")
    success = create_smart_mapper_file()
    
    if success:
        print("🎉 SmartModelPathMapper 생성 완료!")
        print("📁 위치: backend/app/ai_pipeline/utils/smart_model_mapper.py")
    else:
        print("❌ 생성 실패")